...
27. [统一游戏设置规范](#27-统一游戏设置规范)
28. [在线状态与实时通道规范](#28-在线状态与实时通道规范)
...
| `repositoryMode` | 行为摘要 |
|------------------|----------|
| `local` | 纯本地；`networkMode` 视为 offline；不启用在线状态刷新、WebSocket、在线用户人数刷新；除健康检查外不发送远端业务请求 |
| `auto` | 探测远端可用性；可用则获取当前用户并启动在线运行时，否则 offline |
| `remote` | 强制走远端（需 healthCheck 通过），获取当前用户后启动在线运行时 |

相关实现参考：`dataModeService.js`、`remoteGate.js`（`canFetchRemote()`）、`api/request.js`、`realtimeService.js`、`websocketClient.js`。
...
| `constants/` | `GAME_REGISTRY`、`GAME_SEED_CONFIG`、`GAME_SETTING_DEFINITIONS`、`injectionKeys` |
...
| `gameSettingService` | 顶栏游戏设置分组展示、`setHubGameSettingSwitch`（读写在 `userService`） |
| `gameSessionService` | `createGameSession`：对局会话、结算、对局/成绩入队 |
| `syncService` | 云同步、`flushPendingIfOnline`、`refreshRemoteAfterSettle` |
| `onlineService` | 在线状态标记、在线用户列表、在线状态定时刷新 |
| `realtimeService` | 平台单一 WebSocket 实时通道连接、发送、订阅 |
...
- `api/request.js` 必须作为本地模式远端业务请求兜底：`repositoryMode === 'local'` 时仅允许健康检查，其它远端业务请求返回可识别错误且不发送 HTTP。
...
本地模式下不调用 `online/status`、`online/users`，不建立 WebSocket；在线模式在健康检查成功并获取当前用户后启动 `markOnline()`、在线状态刷新、WebSocket 与在线用户人数刷新。
...
定义源：`src/constants/gameSettingDefinitions.js`（`GAME_SETTING_DEFINITIONS`）。顶栏 `GameHubPage` 经 `gameSettingService` 展示**全部**已注册项，**不**随当前游戏过滤。
...
```js
{
  ...
  settings: [
    {
      key: 'highlightAroundCells',   // settingKey
      label: '周围格高亮',
      type: 'switch',
      defaultValue: true,
      description: '...'
...
新增游戏设置**必须**加入上述列表；`key` 即持久化与 API 中 `setting` 对象的键名。

### 27.2 读写与同步链路

| 层级 | 职责 |
|------|------|
| Page / `GameHubPage` | 触发变更；游戏页通过 `*Service` 读开关，**不**自持配置状态 |
| `gameSettingService` | 顶栏分组与 `setHubGameSettingSwitch` |
| `userService` | `readGameSettingBoolean`、`updateGameSetting`、`queueSettingsSync` |
| `userRepository` | `patchGameSettingForCurrentUser`、`collectGameSetting`、`buildGameSettingPendingEvent`、`persistAllLocal` |
| `userStore` | `gameSettings[gameCode][settingKey]` 响应式快照 |
| `syncRepository` | `appendPendingEvent`（`user_game_setting_update`） |
| `syncService` | `flushPendingIfOnline` → `POST /sync/cloud-save` |

```
Page → Service → Repository → Local Cache → Sync Queue → Backend
```
...
| 规则 | 说明 |
|------|------|
| 顶栏统一展示 | 所有 `GAME_SETTING_DEFINITIONS` 中的 `switch` 在顶栏设置模态展示 |
| 禁止页内独占设置 | 不允许只在当前 `{Game}Page` 或 `GameControlPanel` 展示已注册项 |
| `gameCode` + `settingKey` 隔离 | 读写必须带 `gameCode`；键名全局在单游戏 `setting` 对象内唯一 |
| 经 Service 读 | 游戏页用 `userService` 或 `games/*Service` 封装，不直接读 store 拼业务默认值 |
| 经 Service 写 | `userService.updateGameSetting(gameCode, patch)`，不页面直调 API |

### 27.4 禁止

| # | 禁止项 |
|---|--------|
| 1 | 页面直接 `fetch` `GET/PUT .../games/{gameCode}/setting` |
| 2 | 页面直接 `localStorage` 存游戏偏好 |
| 3 | Store 内调 API 或 `appendPendingEvent` |
| 4 | 公共组件、`game-templates` 内 `if (gameCode === 'xxx')` 控制设置展示或默认值 |
| 5 | 各游戏自建与 `gameSettings` 平行的第二套存储或键名表 |
| 6 | 未注册 `gameSettingDefinitions` 却在顶栏/页面硬编码新开关 |

游戏内 `*PageSettings.js` 仅复用 `findGameSettingDefinition` 的文案元数据；逻辑默认值以定义中 `defaultValue` 为准。详见 [architecture.md §2.8](../architecture.md#28-统一游戏设置game-settings)、[new-game-guide.md §9](../new-game-guide.md#9-新增游戏设置)、[api.md §4.5–4.6](../api.md#45-获取用户游戏设置)。
...
- 在线状态必须通过 `services/onlineService.js` 编排。
- HTTP 请求封装在 `api/onlineApi.js`，页面不直接调用 `fetch` 或在线接口。
- `markOnline()` 用于写入在线状态，`markOffline()` 用于删除在线状态，`startOnlineRefresh()` 每 5 分钟刷新一次。
- 在线刷新定时器必须避免重复启动，并在退出登录或销毁在线运行态时清理。
- 在线用户展示组件必须通过 `onlineService.loadOnlineUsers()` 获取数据，不直接调用 `fetch` 或 `onlineApi`。
- 在线人数 badge 必须由在线用户列表长度驱动，在线模式下组件初始化后主动加载并定时刷新，不得依赖弹窗打开才查询。
- 本地模式下必须停止在线状态刷新、停止在线用户人数刷新、不调用 `online/status` 或 `online/users`；切换到本地时先尝试 `markOffline()`，再清理在线运行时并进入本地模式。
- 切换到在线模式时必须健康检查并获取当前用户，服务端用户 ID 有效后再 `markOnline()`、启动在线状态刷新、连接 WebSocket、恢复在线用户人数刷新。
- 在线用户列表仅展示昵称与基于 `onlineAt` 计算的在线时长；不得展示 `username`、登录账号、`serviceId`、手机号、邮箱、token 等账号或敏感字段。
- 在线时长只使用 `onlineAt` 计算，`lastActiveAt` 仅表示最后活跃 / 刷新时间。

### 28.2 实时通道

- 全站只使用 `/ws/game-hub/realtime` 一条 WebSocket。
- 页面不直接 `new WebSocket`，统一通过 `services/realtimeService.js`。
- 底层连接、重连、心跳、按 `type` 分发由 `utils/websocketClient.js` 封装。
- 本地模式、缺少有效 `serviceId`、手动关闭、身份/权限失败状态均不得创建或自动重连 WebSocket。
- close code `1008` 表示身份/权限失败，不自动重连；重新获取当前用户或切换到在线模式后清理失败状态并允许主动连接。
- 普通网络断开由 `onclose` 统一触发指数退避重连，初始 1s，最大 30s；成功连接后重置退避状态。
- `disconnect()` 必须清理重连定时器、心跳定时器和 socket 引用；已有 OPEN / CONNECTING 连接时不得重复创建。
- 消息类型集中在 `constants/realtimeMessageTypes.js`，不同业务通过 `type` 分发。
