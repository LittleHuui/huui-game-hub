...
import { applyUnoConfig } from '../games/uno/unoConfig.js';
...
const GAME_CONFIG_HANDLERS = {
  ...
  [UNO]: {
    async loadRemote() {
      const data = await gameConfigRepository.fetchGameConfig(UNO);
      applyUnoConfig(data);
    },
    applySeed() {
      applyUnoConfig(getSeedGameConfig(UNO, GAME_SEED_CONFIG));
    }
  }
};
