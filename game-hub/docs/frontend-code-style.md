...
20. [命名规范](#20-命名规范)
21. [禁止事项](#21-禁止事项)
22. [新游戏接入规范](#22-新游戏接入规范)
23. [Cursor 执行规则](#23-cursor-执行规则)
24. [游戏页面统一布局规范](#24-游戏页面统一布局规范)
25. [游戏公共组件规范](#25-游戏公共组件规范)
26. [游戏页面数据流规范](#26-游戏页面数据流规范)
27. [游戏页面禁止事项](#27-游戏页面禁止事项)
28. [新游戏接入流程](#28-新游戏接入流程)
...
- 调用 `gameLifecycleService.activateGame`、`gameSessionService` 等。
...
| 时机 | 应做 | 不应做 |
|------|------|--------|
| `onMounted` | `activateGame(gameCode, { difficultyCode, includeLeaderboard })` | 循环请求所有 difficulty 的排行榜 |
| `onUnmounted` | 取消动画 RAF、解绑监听 | — |
| 难度切换 | 带 `difficultyCode` 刷新单榜 | `fetchAllLeaderboards` |
...
- 游戏遗留样式隔离：`minesweeper-legacy.css` 等，**禁止** 污染全局未加前缀的选择器。
...
- **统一** 在 `localRepository.LS_KEYS` 注册；禁止在业务文件散落字符串 key。
- 迁移旧 key 时在 Repository 层做 **一次性** 兼容读取（见 `LEGACY_*`）。
...
> **现状说明**：`gameLifecycleService` 内仍有 minesweeper/match3 分支，新游戏接入时应优先扩展 `GAME_REGISTRY` + 通用 `loadGameShop(gameCode)`，减少复制粘贴。
...
- `settingStore.storageVersion` 升级时在 **boot** 或专用迁移 service 处理，不散落在各页面。
...
| `auto` / `remote` 且 online | 允许 `rankingService.refreshGameLeaderboard` | 允许 |
...
参考：`rankingRepository.fetchLeaderboardDifficulty`、`rankingService.refreshGameLeaderboard`。

### 14.2 允许 / 禁止

| 允许 | 禁止 |
|------|------|
| 用户切换到某难度后拉该难度榜 | 一次 API 拉取所有游戏排行榜 |
...
- **开局 / 结算 / 放弃** 由 `gameSessionService` 编排。
...
| gameCode / difficultyCode | snake_case 字符串：`'minesweeper'`、`'easy'` |
...
- [ ] 在 `GAME_REGISTRY` 注册 `code`、`modes`、`capabilities`。
- [ ] 在 `GAME_SEED_CONFIG.games` 增加种子（difficulties、propRules、items）。
- [ ] 创建 `src/games/{gameCode}/` 目录结构（§2.3）。
- [ ] 实现 `{Game}Page.vue`、`{game}Engine.js`、`{game}Config.js`。
- [ ] 路由注册 + `GameSelector` 入口。
- [ ] 在 `gameLifecycleService.activateGame` 增加 **通用** 分支（优先复用 `loadGameShop(gameCode)`）。
- [ ] 若需排行榜：确保 `leaderboard` capability + 按 difficulty 刷新。
- [ ] 若需道具：走 `inventoryService` / `shopService`，不改平台组件契约。
- [ ] README 简述 mode、config 字段。

### 22.2 最小 Engine 测试思路

- 纯函数：给定 board + config，断言 `animationSteps` 与 score。
- 不启动 Vue 即可在 Node 或 Vitest（若后续引入）中跑。

### 22.3 禁止

- 复制 `Match3Page.vue` 800 行再改名字。
- 在 `App.vue` / `GameHubPage` 写游戏特有逻辑。
...
**禁止** 各游戏单独实现完全不同的大厅式左右布局（旧版 `match3-layout` / 裸 `layout` 类仅作迁移参考，新代码不得新增）。
...
- 游戏编排（开局、结算、模式切换）
- 动画播放与棋盘交互
- 调用 `service`（`gameSessionService`、`purchaseService`、`rankingService` 等）
...
### 25.3 GameRankingPanel

- **props**：`gameCode`、`mode`、`difficultyCode`、`limit`、`subtitle`
- **刷新**：`rankingService.refreshGameLeaderboard`（mounted / props 变化）
- **展示**：`rankingStore.listForDifficulty`
- 离线不请求；无 `leaderboard` capability 时显示「暂无排行榜」

### 25.4 旧组件

`RankingPanel.vue`、`InventoryPanel.vue`、各游戏内 `*Shop` / `*Leaderboard` 可保留文件，**新页面不得再引用**；逐步淘汰。
...
背包：inventoryStore（流水）
        ...
        → Page @use-prop → gameSessionService / 游戏逻辑

排行榜：rankingService.refreshGameLeaderboard(gameCode, difficultyCode, mode)
        → rankingStore
        → GameRankingPanel
...
在 [§22](#22-新游戏接入规范) 基础上，**必须** 额外完成：

1. `GAME_SEED_CONFIG` + `GAME_REGISTRY` 注册（含 `shop` / `inventory` / `leaderboard` capability）。
2. 创建 `src/games/{gameCode}/`（`Page`、`Engine`、`Config`）。
3. `{Game}Page.vue` 使用 `GamePlayLayout` 及六个公共面板组件。
4. `gameLifecycleService.activateGame` 中复用 `loadGameShop(gameCode)`、`refreshGameBag`、`refreshGameLeaderboard`。
5. 算法仅放在 `{game}Engine.js`；页面不写复杂算法、不 `fetch`、不 `localStorage`。

> **默认规则**：后续 Cursor 前端任务以本文 §24–§28 为游戏页默认约束，无需在 prompt 中重复描述布局与商城/背包/榜拆分。
...
*文档版本：1.1 · 与 game-hub `src/` 现状对齐（Vue 3 + Pinia + Vite · 含统一游戏页布局 §24–§28）*
